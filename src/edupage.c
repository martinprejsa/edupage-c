...
size_t buffer_writer(void *buffer, size_t size, size_t nmemb,
                     struct buffer *userp) {
...
struct EdupageClient *edupage_client_create(char const username[static 1],
                                            char const password[static 1],
                                            char const server[static 1]) {
...
void edupage_canteen_destroy(struct EdupageCanteen *canteen) {
    for (int index = 0; index < canteen->count; index++) {
      free(canteen->meal_plan[index]);
    }
...
edupage_client_get_meals(struct EdupageClient const client[static 1],
                         const time_t t) {
  CURL *handle = curl_easy_init();
  ...
  struct EdupageCanteen * canteen = malloc(sizeof(struct EdupageCanteen));
  canteen->count = 0;

  size_t index = 0;
  json_object_foreach(ticket, key, value) {
    if (strcmp(key, "addInfo")) {
      json_t *singleton = json_object_get(value, "2");
      ...
      char const * data = json_string_value(meal);
      if (data == nullptr) {
        error_callback(EDUPAGE_ERR_INVALID_PARAMETER, "meal not found");
        json_decref(root);
        return nullptr;
      ...
      canteen->count++;
      canteen = realloc(canteen, sizeof(struct EdupageCanteen) + canteen->count * sizeof(char*));
      canteen->meal_plan[index] = strndup(data, strlen(data)-1);
